...
osa_status_t OSA_MsgQCreate(osa_msgq_handle_t msgqHandle, uint32_t msgNo)
{
    ...
    xMsgqHandle.msgq = xQueueCreate(msgNo, sizeof(uint32_t *));
...
void OSA_InterruptEnable(void)
{
    if (__get_IPSR() != 0U)
    {
        if (s_osaState.basePriorityTop != 0)
        {
...
void OSA_InterruptDisable(void)
{
    if (__get_IPSR() != 0U)
    {
        if ((uint32_t)s_osaState.basePriorityTop < OSA_MAX_ISR_CRITICAL_SECTION_DEPTH)
        {
